...
<?php
    ...
    if (isset($_REQUEST['name'])&& isset($_REQUEST['fullname'])&&isset($_REQUEST['password'])&&isset($_REQUEST['phone'])&&isset($_REQUEST['email'])&&isset($_REQUEST['adress'])){
        ...
        $sql = "INSERT into `users` (username, fullname, password,phone,address, email, created_at,updated_at) VALUES ('$name', '$fullname','".md5($password)."','$phone','$adress', '$email', '$created_at','$updated_at)')";
