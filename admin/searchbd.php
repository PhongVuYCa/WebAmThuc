...
<?php
    ...
    $sql="select  month(ngaygiao) as thang,sum(thanhtien) as tongtien  from doanhthu where tinhtrang='Đã giao' and year(ngaygiao)='$nam' group by thang";
    $result = $conn->query($sql);
    $chart_data = '';
    while($row = $result->fetch_assoc()) { 

        $chart_data .= "{  thang:".$row["thang"].", Tổng:".$row["tongtien"]."}, ";
     }
