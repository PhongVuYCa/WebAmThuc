...
<?php
    ...
    $sql = "SELECT sanpham.id,thumbnail, titlesp,price,sum(doanhthu.soluong) as soluongban ,sum(thanhtien) as tongtien from sanpham  join doanhthu on doanhthu.idsp=sanpham.id where doanhthu.tinhtrang='Đã giao' and month(ngaygiao)= '$thang' and year(ngaygiao)= '$nam'  group by sanpham.id,thumbnail, titlesp,price";
